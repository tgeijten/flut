...
namespace flut
{
	struct pattern_matcher
	{
		...
		bool match( const string& str ) const { for ( auto& p : patterns ) if ( pattern_match( str, p ) ) return true; return false; }

		bool operator()( const string& str ) const { return match( str ); }
		template< typename T > bool operator()( const T& str ) const { return match( static_cast< const string& >( *str ) ); }
		template< typename T > bool operator()( const T* str ) const { return match( static_cast< const string& >( *str ) ); }
